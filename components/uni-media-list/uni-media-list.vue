<template>
    <view>
        <view class="list-cell" hover-class="uni-list-cell-hover" @click="bindClick">
            <view class="media-list" v-if="data.name">
                <view class="isImgLeft">
                    <text class="media-title2">{{data.name}}</text>
                    <text class="media-title2">{{data.sort_desc}}</text>
                    <!-- <view class="image-section-left">
                        <image class="image-list2" src="https://img.cdn.aliyun.dcloud.net.cn/dev/img/ext/git.png"></image>
                    </view> -->
                </view>
                <view class="media-foot">
                    <view class="media-info">
                        <text class="info-text">{{data.category}}</text>
                        <!-- <text class="info-text">{{data.comment_count}}条评论</text> -->
                        <text class="info-text">{{data.update_time}}</text>
                    </view>
