		...
		<scroll-view class="list" v-for="(tabItem, idx) in newsList" :key="idx" v-if="tabIndex === idx" scroll-y
		 @scrolltolower="loadMore(idx)">
			...
			<view class="uni-tab-bar-loading">
				<view class="loading-more">{{tabItem.loadingText}}</view>
			</view>
		</scroll-view>
		<!-- #endif -->
	</view>
			...
			getCategory() {
				uni.request({
					...
					success: (result) => {
						...
						this.tabBars['6'].name = 'JS 项目';
						this.tabBars['8'].name = 'Obj-C 项目';
						if (this.hasLike) {
							let tabBars1 = [];
							let tabBars2 = [];
							...
							this.likeTabs.forEach((likeTab) => {
								tabBars2.push({
									id: likeTab,
									name: tabBars1[likeTab]
								})
							})
							this.tabBars = tabBars2;
						}
						this.tabBars.forEach((tabBar) => {
							this.newsList.push({
								data: [],
